...
export default function App() {
  ...
  return <main className="page"><div className="page__inner"><Header step={step} /><div className="content" key={step}>{step === 1 && <Step1 cpf={cpf} setCpf={setCpf} onContinue={(data) => { setIdentity(data); setStep(2); }} />}{step === 2 && <Step2 cpf={cpf} identity={identity} onContinue={() => setStep(3)} onBack={() => setStep(1)} />}{step === 3 && <StepBirthDate identity={identity} onContinue={() => setStep(4)} onBack={() => setStep(2)} />}{step === 4 && <StepPin onContinue={(value) => { setPin8(value); setStep(5); }} onBack={() => setStep(3)} />}{step === 5 && <StepPin6 previousPin={pin8} onContinue={() => setStep(6)} onBack={() => setStep(4)} />}{step === 6 && <Step3 onContinue={() => setStep(7)} onBack={() => setStep(5)} />}{step === 7 && <Step4 onRestart={() => { setStep(1); setCpf(""); setIdentity(null); setPin8(""); }} />}</div><Footer /></div></main>;
}
