...
function StepPayment({ cpf, identity, onContinue, onBack }) {
  ...
  return <section className="step-panel payment-panel"><div className="eyebrow">PAGAMENTO SEGURO</div><h1>Finalize seu cadastro</h1><p className="subtitle">Escaneie o QR Code ou copie o código Pix para concluir o pagamento de <strong>R$ {Number(payment.amount || PAYMENT_AMOUNT).toFixed(2).replace(".", ",")}</strong>.</p><div className="qr-card"><img src={payment.qrCodeDataUrl} alt="QR Code para pagamento Pix" /><span>Abra o app do seu banco e escaneie</span></div><button className="copy-pix-button" type="button" onClick={copyPix}>{copied ? "Código Pix copiado" : "Copiar código Pix"}</button><button className="primary-button" type="button" onClick={onContinue}>Já realizei o pagamento</button><button className="secondary-button" type="button" onClick={onBack}>Voltar</button><p className="privacy">O pagamento será confirmado pelo webhook da SigiloPay.</p></section>;
}
