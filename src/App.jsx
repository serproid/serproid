...
function StepPin({ onContinue, onBack }) {
  ...
  function submit(event) { event.preventDefault(); setTouched(true); if (valid) onContinue(pin); }
  return <form className="step-panel pin-panel" onSubmit={submit}><div className="illustration pin-illustration"><span>♢</span></div><h1>Crie seu PIN de 8 dígitos</h1><p className="subtitle">Use 8 números que você lembre facilmente</p><label className="pin-label" htmlFor="pin">PIN de 8 dígitos</label><input id="pin" className="pin-input" type="password" inputMode="numeric" autoComplete="new-password" maxLength={8} value={pin} placeholder="••••••••" onChange={(event) => setPin(event.target.value.replace(/\D/g, ""))} onBlur={() => setTouched(true)} />{touched && !valid && <p className="error-message centered-error">Digite exatamente 8 dígitos.</p>}<button className="primary-button" type="submit" disabled={!valid}>Continuar <span className="button-arrow">→</span></button><button className="secondary-button" type="button" onClick={onBack}>Voltar</button><p className="privacy">Não compartilhe seu PIN com outras pessoas.</p></form>;
}
...
export default function App() {
  ...
  return <main className="page"><div className="page__inner"><Header step={step} /><div className="content" key={step}>{step === 1 && <Step1 cpf={cpf} setCpf={setCpf} onContinue={(data) => { setIdentity(data); setStep(2); }} />}{step === 2 && <Step2 cpf={cpf} identity={identity} onContinue={() => setStep(3)} onBack={() => setStep(1)} />}{step === 3 && <StepBirthDate identity={identity} onContinue={() => setStep(4)} onBack={() => setStep(2)} />}{step === 4 && <StepPin onContinue={() => setStep(5)} onBack={() => setStep(3)} />}{step === 5 && <Step3 onContinue={() => setStep(6)} onBack={() => setStep(4)} />}{step === 6 && <Step4 onRestart={() => { setStep(1); setCpf(""); setIdentity(null); }} />}</div><Footer /></div></main>;
}
