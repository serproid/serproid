...
function Step2({ cpf, onContinue, onBack }) { const masked = useMemo(() => `${cpf.slice(0, 3)}.***.***-${cpf.slice(-2)}`, [cpf]); return <section className="step-panel"><div className="illustration"><span>✓</span></div><h1>Confirme seus dados</h1><p className="subtitle">Encontramos um cadastro para o CPF informado. Confira antes de continuar.</p><div className="summary-card"><span>CPF</span><strong>{masked}</strong><span className="verified">✓ verificado</span></div><button className="primary-button" type="button" onClick={onContinue}>Confirmar e continuar</button><button className="secondary-button" type="button" onClick={onBack}>Voltar</button><p className="privacy">Você está no ambiente seguro SerproID</p></section>; }
